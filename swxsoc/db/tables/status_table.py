...
def _build_class() -> Any:
    ...
    status_origin_association = Table(
        ...
        Column(
            "status_id",
            Integer,
            ForeignKey(f"{mission_name}_status.status_id"),
            primary_key=True,
        ),
        Column(
            "origin_file_id",
            Integer,
            ForeignKey(f"{mission_name}_science_file.science_file_id"),
            primary_key=True,
        ),
    )

    class StatusTable(Base.Base):  # type: ignore
        __tablename__ = f"{mission_name}_status"
        ...
        original_processing_timestamp = Column(
            DateTime, nullable=False, default=datetime.now(timezone.utc)
        )
        last_processing_timestamp = Column(
            DateTime, nullable=False, default=datetime.now(timezone.utc)
        )
        ...
        def __init__(
            ...
            self.original_processing_timestamp = (
                original_processing_timestamp or datetime.now(timezone.utc)
            )  # type: ignore[assignment]
            self.last_processing_timestamp = last_processing_timestamp or datetime.now(
                timezone.utc
            )  # type: ignore[assignment]
